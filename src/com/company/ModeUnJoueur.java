 ...
 */
class ModeUnJoueur {
    ModeUnJoueur(String name1, Scanner sc1) {

        String name2 = "ORDINATEUR";
        int tab[]={0,0,0,0,0}, tab1[]={0,0,0,0,0,0};
        int ch= (new Choix()).choix(true,name1, sc1);
        ...
        while ((joueur1.getVie() > 0) && (ordi.getVie() > 0)) {
            tab = joueur1.getCathegoPersonnage().attaque(sc1,joueur1.getVie(), ordi.getVie(), name1, name2,ch, false, joueur1.getForce(), joueur1.getAgilité(), ordi.getIntelligence(), joueur1.getIntelligence());
            joueur1.setVie(tab[0]);
            ordi.setVie(tab[1]);
            joueur1.setAgilité(tab[2]);
            if ((joueur1.getVie() > 0) && (ordi.getVie() > 0)) {
                tab = ordi.getCathegoPersonnage().attaque(sc1, ordi.getVie(), joueur1.getVie(), name2, name1, tab1[5], true, ordi.getForce(), ordi.getAgilité(), joueur1.getIntelligence(), ordi.getIntelligence());
                ordi.setVie(tab[0]);
